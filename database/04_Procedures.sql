...
-- ============================================================================
-- PACKAGE: ADM_USER_PROFILE_PKG
-- Purpose: Manages operations related to user profile data
-- ============================================================================
CREATE OR REPLACE PACKAGE ADM.ADM_USER_PROFILE_PKG AS
    TYPE ref_cursor_type IS REF CURSOR;
     ...
     * @return Cursor with all user profile data
     */
    ...
    /**
     * Gets all phones for a user
     * 
     * @param p_user_id ID of the user
     * @return Cursor with all user phones
     */
    ...
    /**
     * Gets all emails for a user
     * 
     * @param p_user_id ID of the user
     * @return Cursor with all user emails
     */
    FUNCTION get_user_emails(
        p_user_id IN NUMBER
    ) RETURN ref_cursor_type;
END ADM_USER_PROFILE_PKG;
/

CREATE OR REPLACE PACKAGE BODY ADM.ADM_USER_PROFILE_PKG AS
    FUNCTION get_user_profile_data(
        p_user_id IN NUMBER
    ) RETURN ref_cursor_type
    IS
    ...
    BEGIN
        OPEN v_cursor FOR
            SELECT 
                ...
                u.username
            FROM PU.PERSONUSER u
            JOIN ADM.PERSON p ON u.person_id = p.id
            JOIN ADM.TYPE_IDENTIFICATION ti ON p.type_identification_id = ti.id
            JOIN ADM.GENDER g ON p.gender_id = g.id
            JOIN PU.INSTITUTION_PERSON ip ON p.id = ip.person_id
            JOIN ADM.INSTITUTION i ON ip.institution_id = i.id
    ...
    BEGIN
        OPEN v_cursor FOR
            SELECT 
                p.phone_number,
                tp.name as phone_type_name
            FROM PU.PERSONUSER u
            JOIN ADM.PERSON per ON u.person_id = per.id
            JOIN PU.PHONE_PERSON pp ON per.id = pp.person_id
            JOIN PU.PHONE p ON pp.phone_id = p.id
            JOIN ADM.TYPE_PHONE tp ON p.type_phone_id = tp.id
            WHERE u.id = p_user_id;
    ...
    BEGIN
        OPEN v_cursor FOR
            ...
            FROM PU.PERSONUSER u
            JOIN ADM.PERSON p ON u.person_id = p.id
            JOIN PU.EMAIL e ON p.id = e.person_id
            JOIN ADM.DOMAIN d ON e.domain_id = d.id
            WHERE u.id = p_user_id;
...
-- Grants para el paquete de perfil de usuario
GRANT EXECUTE ON ADM.ADM_USER_PROFILE_PKG TO PU;
